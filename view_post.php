   ...
   if($user_id != ''){
    ...
    //   $tutor_id = $fetch_content['tutor_id'];
      ...
      if($select_likes->rowCount() > 0){
         ...
         $insert_likes = $conn->prepare("INSERT INTO `likes`(user_id, post_id,content_id) VALUES(?,?,0)");
   ...
   if($user_id != ''){
    ...
    //   $tutor_id = $fetch_content['tutor_id'];
      ...
      if($select_content->rowCount() > 0){
         ...
         $select_comment = $conn->prepare("SELECT * FROM `commentss` WHERE post_id = ? AND user_id = ?  AND comment = ?");
         $select_comment->execute([$content_id, $user_id, $comment_box]);

         if($select_comment->rowCount() > 0){
            $message[] = 'comment already added!';
         }else{
            $insert_comment = $conn->prepare("INSERT INTO `commentss`(id, post_id, user_id, comment) VALUES(?,?,?,?)");
...
if(isset($_POST['delete_comment'])){
   ...
   $verify_comment = $conn->prepare("SELECT * FROM `commentss` WHERE id = ?");
   $verify_comment->execute([$delete_id]);

   if($verify_comment->rowCount() > 0){
      $delete_comment = $conn->prepare("DELETE FROM `commentss` WHERE id = ?");
...
if(isset($_POST['update_now'])){
   ...
   $verify_comment = $conn->prepare("SELECT * FROM `commentss` WHERE id = ? AND comment = ?");
   $verify_comment->execute([$update_id, $update_box]);

   if($verify_comment->rowCount() > 0){
      $message[] = 'comment already added!';
   }else{
      $update_comment = $conn->prepare("UPDATE `commentss` SET comment = ? WHERE id = ?");
...
<?php
   if(isset($_POST['edit_comment'])){
      ...
      $verify_comment = $conn->prepare("SELECT * FROM `commentss` WHERE id = ? LIMIT 1");
...
<section class="edit-comment">
   ...
   <form action="" method="post">
      ...
      <div class="flex">
         <a href="view_post.php?get_id=<?= $get_id; ?>" class="inline-option-btn">cancel edit</a>
...
<section class="comments">
   ...
   <div class="show-comments">
      <?php
         $select_comments = $conn->prepare("SELECT * FROM `commentss` WHERE post_id = ?");
